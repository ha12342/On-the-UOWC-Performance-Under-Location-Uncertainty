...
z_sao_1 = @(y) z_sao(sqrt(y)./sqrt(y_t));
...
denta_1_1 = @(y) denta_1(sqrt(y)./sqrt(y_t));
denta_2 = @(y)   denta_tong(3,R*c_av*z_min(y)./(2*H_1),e_2*(m+3)/2,uu_3,m) + denta_tong(4,R*c_av*z_min(y)./(2*H_1),e_2*(m+3)/2,uu_3,m);


f_y_1 = @(y) erfc(sqrt(y/2)).*(3*(H_1^2*z_sao(y).^3 - R^2 * z_min(y).^3)./(4*H_1^2*H_2*R^2*(m+1)) .* y.^(-1) - (e_1*denta_1(y))./(2.*y_t.^(1/(2*m+2))*(m+1)).*y.^(-(2*m+1)/(2*m+2)));

f_y_2 = @(y) erfc(sqrt(y/2)).*(-(3*z_min(y).^3./(4*H_1^2*H_2*(m+1))).*y.^-1 +(3*e_3/(4.*y_t.^(1/(2*m+2))*(m+1)) - e_1*denta_2(y)./(2.*y_t.^(1/(2*m+2))*(m+1))).*y.^(-(2*m+1)/(2*m+2)));

I = 1/2 *(integral(f_y_1,y_1,y_max) + integral(f_y_2,y_min,y_1));
disp(I);
